<template>
  <transition name="app-drawer">
    <div v-if="value" class="app-drawer-layer">
      <button class="app-drawer__backdrop" type="button" aria-label="إغلاق" @click="close" />
      <aside class="app-drawer" :class="`app-drawer--${side}`" role="dialog" aria-modal="true">
        <header class="app-drawer__header">
          <h2>{{ title }}</h2>
          <AppIconButton aria-label="إغلاق" variant="neutral" @click="close">
